...
| `declares payload-sensitive step pattern ... with a wildcard step pattern` / `declares a wildcard step pattern with a payload-sensitive state match step pattern` / `declares payload-sensitive step pattern ... with a state match wildcard step pattern` | A payload-sensitive state-match split is mixed with a non-state-match wildcard fallback, or a state-match wildcard fallback is mixed with a non-state-match payload-sensitive clause. | Use state-match bodies for both the explicit payload-sensitive clauses and the wildcard fallback, use supported step-signature or `match msg` fallback where that surface is intended, or remove the wildcard or payload predicate. |
