...
| `step return match arm cannot perform for loops` / `step return match arm cannot perform nested for loops` | A `step return match` arm tries to run a loop inside an arm-local runtime branch or another loop. | Keep the arm-local loop direct, over a runtime `List<T,N>` binding, and keep any loop-body runtime branch action-only. |
