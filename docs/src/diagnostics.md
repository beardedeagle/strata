...
| `step return match arm cannot perform more than one runtime if` / `step return match arm cannot perform nested runtime if` | A `step return match` arm tries to add more branch nesting than this selected-arm prefix slice admits. | Keep at most one direct statement-level runtime `if` in an arm, keep its branches to local `emit` or in-scope direct `send` actions, and keep runtime branches outside arm-local loop bodies. |
| `step return match arm cannot perform more than one for loop` | A `step return match` arm tries to run multiple direct bounded loops inside one selected-arm prefix. | Keep at most one direct bounded `for` loop in an arm. |
| `step return match arm cannot perform for loops` / `step return match arm cannot perform nested for loops` | A `step return match` arm tries to run a loop inside an arm-local runtime branch or another loop. | Keep the arm-local loop direct, over a runtime `List<T,N>` binding, and keep the loop body to local `emit` or in-scope direct `send`. |
