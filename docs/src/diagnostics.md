...
| `step return match arm cannot bind process reference ...` | A `step return match` arm tries to acquire branch-local authority with `spawn`. | Bind process references before `return match`, or keep the arm prefix to admitted `emit` / in-scope direct `send` actions. |
| `step return match arm cannot perform runtime if` / `step return match arm cannot perform for loops` | A `step return match` arm tries to add nested runtime control flow in the selected-arm prefix. | Keep arm-local prefixes to admitted straight-line actions, or split the behavior into explicit step clauses / whole-body dispatch. |
