...
| `step return match arm cannot bind process reference ...` | A `step return match` arm tries to acquire branch-local authority with `spawn`. | Bind process references before `return match`, or keep the arm prefix to admitted `emit`, in-scope direct `send`, or one direct action-only runtime `if`. |
| `step return match arm cannot perform more than one runtime if` / `step return match arm cannot perform nested runtime if` | A `step return match` arm tries to add more branch nesting than this selected-arm prefix slice admits. | Keep at most one direct statement-level runtime `if` in an arm, and keep its branches to local `emit` or in-scope direct `send` actions. |
| `step return match arm cannot perform for loops` | A `step return match` arm tries to run bounded iteration inside the selected-arm prefix. | Move iteration outside the return-match arm, or split the behavior into explicit step clauses / whole-body dispatch. |
