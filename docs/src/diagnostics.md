...
| `effect outcome binding ... must appear before ordinary effect statements` | A later outcome binding would be executed before an earlier ordinary effect at runtime. | Bind local effect outcomes before ordinary `emit`, `send`, `if`, or `for` statements. |
