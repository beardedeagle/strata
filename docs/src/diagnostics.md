...
| `declares overlapping collection patterns ...` | Exact and subset collection patterns could match the same concrete value. | Make map subset patterns disjoint, use a single exact pattern, or add a wildcard fallback for the non-overlapping remainder. |
