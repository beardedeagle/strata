...
| `step return match must not follow runtime effect statements` | A `step` performs `emit`, `spawn`, or `send` before `return match`, which would require per-arm effect ordering semantics. | Keep `step return match` pure in this slice, or use explicit step clauses / whole-body dispatch. |
