...
| `duplicates spawn target` | A transition spawns the same target twice. | Spawn a target at most once per message transition. |
| `sends to ... before it is spawned` | A transition sends to a process before spawning it. | Spawn before sending. |
| `sends to ..., which is not running` | A transition sends to a stopped process. | Do not send after the target can stop. |
