...
| `type name ... is reserved` | A source record or enum tries to use a built-in transition, capability, collection, primitive, outcome, or scalar type name. | Rename the source type. |
...
| `... conflicts with core Bool type` / `... conflicts with core Bool value constructor` / `... conflicts with built-in primitive type name` | A user declaration tries to define `Bool`, `True`, `False`, `String`, or `Bytes`, which are Strata core built-ins. | Rename the user declaration; `Bool`, `True`, `False`, `String`, and `Bytes` are always available and cannot be redeclared. |
