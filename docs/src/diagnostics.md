...
| `step return match arm cannot bind process reference ...` | A `step return match` arm tries to acquire branch-local authority with `spawn`. | Bind process references before `return match`, or keep the arm prefix to admitted `emit`, in-scope direct `send`, one direct action-only runtime `if`, or one direct bounded runtime `for`. |
| `step return match arm cannot perform more than one runtime if` / `step return match arm cannot perform nested runtime if` | A `step return match` arm tries to add more branch nesting than this selected-arm prefix slice admits. | Keep at most one direct arm-level runtime `if` and at most one direct action-only runtime `if` inside an arm-local loop body. |
| `step return match arm cannot perform more than one for loop` | A `step return match` arm tries to run multiple direct bounded loops in one selected-arm prefix or one arm-local runtime branch. | Keep at most one direct bounded `for` loop in the selected prefix or runtime branch. |
| `step return match arm cannot perform for loops` / `step return match arm cannot perform nested for loops` | A `step return match` arm tries to run a loop inside a loop body, a nested runtime branch, or another unsupported nested context. | Keep the arm-local loop direct, over a runtime `List<T,N>` binding, and keep any loop-body runtime branch action-only. |
...
| `step return match arm nested return match is not supported` | A `step return match` arm tries to nest another `return match`. | Keep the selected arm as a straight-line prefix plus one terminal `Continue(...)`, `Stop(...)`, or `Panic(...)`. |
