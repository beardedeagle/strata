...
| `runtime authority/effect binding source fingerprint does not match artifact source hash` / `mantle_artifact_source_hash_fnv1a64` | A checked authority/effect artifact or runtime binding does not match the `.mta` it is being bound to or run with. | Rebuild the `.mta`, `.authority-effect.json`, and `.authority-effect-binding.json` from the same checked source; do not hand-edit identity fields. |
...
| `field "schema_id" must be "mantle.runtime_authority_effect_binding"` / `authority_effect_schema_id` / `spawn_authority_policy` | Mantle received a malformed, wrong-kind, unsupported-version, or unsupported-policy authority/effect binding. | Supply the output from `strata authority-effects bind-runtime`; `.authority-effect.json` is not Mantle runtime input. |
...
| `--deny-spawn-authority cannot be combined with --authority-effect-binding` | `mantle run` was given both a direct runtime spawn-denial flag and an authority/effect binding policy. | Encode the runtime spawn authority policy in the binding with `strata authority-effects bind-runtime --deny-spawn-authority`, then pass only `--authority-effect-binding` to Mantle. |
