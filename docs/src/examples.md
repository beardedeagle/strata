...
15. `function_record_pattern.str` for source helper record destructuring
   patterns.
16. `function_record_return_match.str` for helper return-match record
   destructuring.
17. `function_record_body_match.str` for whole-body helper match record
   destructuring.
18. `state_payload_enum.str` for payload-bearing process state enum transitions.
19. `collection_state.str` for immutable collection state and payload-dependent
   collection next-state templates.
20. `state_payload_match.str` for matching immutable current process state
   payloads.
21. `actor_instances.str` for multiple runtime instances of one process
   definition.
22. `actor_payloads.str` for typed message payloads and immutable payload
   bindings in actor step parameter patterns.
23. `runtime_if_else.str` for Mantle-backed runtime branching over a message
   payload.
24. `runtime_payload_projection_if.str` for Mantle-backed runtime branching over
   a projected field from an immutable received record payload.
25. `runtime_payload_projection_next_state.str` for Mantle-backed runtime
   next-state branching over a projected field from an immutable received record
   payload.
26. `runtime_state_payload_projection_if.str` for Mantle-backed runtime
   branching over a projected field from an immutable current-state record
   payload.
27. `runtime_state_payload_projection_next_state.str` for Mantle-backed runtime
   next-state branching over a projected field from an immutable current-state
   record payload.
28. `runtime_nested_if_actions.str` for one bounded layer of nested
   statement-level runtime branch actions.
29. `runtime_final_if_guarded_loop.str` for bounded loop action prefixes inside
   final-position runtime branches.
30. `runtime_final_if_nested_if_actions.str` for one direct nested
   statement-level runtime branch action inside final-position runtime
   branches.
31. `runtime_final_if_nested_terminal_if.str` for one direct nested terminal
   final-position runtime branch inside final-position runtime branches.
32. `runtime_guard_noop.str` for omitted `else` and explicit no-op runtime
   branch behavior.
33. `runtime_for_each.str` for Mantle-backed bounded runtime iteration over a
   typed list payload.
34. `runtime_for_each_empty.str` for the zero-iteration runtime collection case.
35. `runtime_for_each_if.str` for Mantle-backed runtime branch selection inside
   bounded loop bodies.
36. `runtime_for_each_nested_if_actions.str` for one bounded nested runtime
   branch inside a bounded loop-body branch.
37. `runtime_guarded_for_each.str` for guarding a whole bounded runtime loop.
38. `runtime_guarded_ref_loop.str` for routing a guarded bounded loop through a
   received direct process reference.
39. `runtime_guarded_ref_loop_jobs.str` for routing ordinary immutable `Job`
   values through a guarded loop and received direct process reference.
40. `runtime_loop_element_projection.str` for projecting immutable record
   fields from guarded runtime loop elements.
41. `actor_payload_match.str` for the same payload binding through a whole-body
   `match msg`.
42. `actor_payload_split_match.str` for payload-sensitive same-message
   splitting inside a whole-body `match msg`.
43. `actor_payload_split_signature.str` for payload-sensitive same-message
   splitting across step parameter patterns.
44. `actor_payload_split_signature_wildcard.str` for payload-sensitive
   step-signature wildcard fallback over discovered concrete payload cases.
45. `actor_payload_state_match_split.str` for payload-sensitive same-message
   splitting across state-match step clauses.
46. `actor_payload_state_match_wildcard.str` for payload-sensitive state-match
   wildcard fallback over discovered concrete payload cases.
47. `nested_patterns.str` for nested immutable constructor, record, list, and
   map payload destructuring.
48. `actor_reply.str` for transporting typed process references through message
   payloads.
49. `actor_emit_spawn_send.str` for one transition with declared emit, spawn,
   and send authority.
50. `actor_panic_no_replay.str` for fail-closed actor failure and no replay
...
- Arm-local `spawn`, process-reference binding, runtime `for`, nested return
  matches, and mutable source state changes remain rejected.
