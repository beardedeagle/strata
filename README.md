...
authorities, required `Cap<Spawn<Target>>` descriptors, and spawn-site IDs
without building an artifact. `mantle inspect-authority` reads an admitted
`.mta` and prints the loaded authority and spawn-site tables without executing
the program. Both commands default to text and support `--format json` for CI,
audit, and review tooling. They are optional inspection commands, not reports
generated by normal build or run.
