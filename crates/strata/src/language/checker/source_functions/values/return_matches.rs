    ...
    for arm in check_payload_sensitive_typed_match_arms(&pattern_context, &match_body.arms)? {
        ...
        } = &arm.pattern
        {
            validate_source_pattern_binding_scope_conflicts(
                scope,
                &format!("function {} return match payload binding", function.name),
                payload_bindings,
            )?;
...
fn validate_source_function_return_record_match(
    ...
    validate_source_pattern_binding_scope_conflicts(
        scope,
        &format!(
            "function {} return match record pattern binding",
            function.name
        ),
        &pattern_bindings,
    )?;
        ...
        match &arm.pattern {
            ...
            Pattern::List(_) | Pattern::Map(_) => {
                ...
                validate_source_pattern_binding_scope_conflicts(
                    scope,
                    &format!(
                        "function {} return match collection pattern binding",
                        function.name
                    ),
                    &pattern_bindings,
                )?;
                ...
                {
                    return Err(Error::new(format!(
                        "function {} return match declares overlapping collection patterns {} and {}",
                        function.name,
