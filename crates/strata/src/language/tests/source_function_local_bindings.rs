...
fn rejects_source_local_binding_type_mismatch_and_unknowns() {
    for (needle, replacement, expected) in [
        (
            ...
            "function route source-local binding current_local must use a declared record, enum, scalar, primitive, list, or map type without process-reference authority, found Missing",
        ),
        (
            "let current_local: Phase = status(work);",
...
fn rejects_process_reference_source_local_binding() {
    ...
    assert!(
        err.to_string().contains(
            "function route source-local binding worker_local must use a declared record, enum, scalar, primitive, list, or map type without process-reference authority, found ProcessRef<Main>"
        ),
        "{err}"
    );
}
