...
impl MantleArtifact {
    ...
    pub(in crate::artifact) fn evaluate_state_value_with_current_state(
        &self,
        template: &ArtifactValueTemplate,
        received_payload: Option<&ArtifactPayload>,
...
pub(in crate::artifact) fn validate_artifact_identity(
    format: &str,
    schema_version: &str,
) -> Result<()> {
...
pub(in crate::artifact) fn validate_unique_process_ref_list(
    process_refs: &[ArtifactProcessRef],
) -> Result<()> {
    let mut seen = BTreeSet::new();
