pub const ARTIFACT_MAGIC: &str = "MTA0";
pub const ARTIFACT_FORMAT: &str = "mantle-target-artifact";
pub const ARTIFACT_SCHEMA_VERSION: &str = "6";
pub const MAX_ARTIFACT_BYTES: usize = 1024 * 1024;
pub const MAX_ARTIFACT_FIELDS: usize = 16_384;
pub const MAX_FIELD_VALUE_BYTES: usize = 16 * 1024;
pub const MAX_IDENTIFIER_BYTES: usize = 128;
pub const MAX_TYPE_COUNT: usize = 4096;
pub const MAX_PROCESS_COUNT: usize = 256;
pub const MAX_STATE_VALUES_PER_PROCESS: usize = 1024;
pub const MAX_MESSAGE_VARIANTS_PER_PROCESS: usize = 1024;
pub const MAX_ENUM_VARIANTS_PER_TYPE: usize = MAX_TYPE_COUNT;
pub const MAX_TRANSITIONS_PER_PROCESS: usize = MAX_MESSAGE_VARIANTS_PER_PROCESS;
pub const MAX_OUTPUT_LITERALS: usize = 4096;
pub const MAX_ACTIONS_PER_PROCESS: usize = 4096;
pub const MAX_DIRECT_RUNTIME_IF_ACTION_DEPTH: usize = 2;
pub const MAX_NEXT_STATE_IF_ELSE_DEPTH: usize = 2;
pub const MAX_EFFECTS_PER_TRANSITION: usize = 3;
pub const MAX_EFFECT_OUTCOMES_PER_TRANSITION: usize = MAX_VALUE_TEMPLATE_FIELDS;
pub const MAX_RUNTIME_FEATURE_REQUIREMENTS: usize = 32;
pub const MAX_PROCESS_REFS_PER_PROCESS: usize = MAX_ACTIONS_PER_PROCESS;
pub const MAX_AUTHORITIES_PER_PROCESS: usize = MAX_ACTIONS_PER_PROCESS;
pub const MAX_SPAWN_SITES_PER_PROCESS: usize = MAX_ACTIONS_PER_PROCESS;
pub const MAX_SUPERVISORS_PER_PROCESS: usize = 16;
pub const MAX_SUPERVISOR_CHILDREN_PER_SUPERVISOR: usize = 128;
pub const MAX_PROTOCOL_COUNT: usize = 256;
pub const MAX_PORT_COUNT: usize = 256;
pub const MAX_COMPONENT_COUNT: usize = 256;
pub const MAX_COMPOSITION_COUNT: usize = 64;
pub const MAX_COMPONENT_INSTANCE_COUNT: usize = 256;
pub const MAX_PORT_BINDING_COUNT: usize = 256;
pub const MAX_MAILBOX_BOUND: usize = 65_536;
pub const MAX_VALUE_TEMPLATE_DEPTH: usize = 64;
pub const MAX_VALUE_TEMPLATE_FIELDS: usize = 256;
