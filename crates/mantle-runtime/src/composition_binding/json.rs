...
use super::COMPOSITION_SCHEMA_SUFFIX;
...
impl<'a> JsonObject<'a> {
    ...
    pub(super) fn required_composition_schema_id(
        &self,
        field: &str,
        source_language: &str,
    ) -> Result<()> {
        let actual = self.required_string(field)?;
        if actual.len() == source_language.len() + COMPOSITION_SCHEMA_SUFFIX.len()
            && actual.starts_with(source_language)
            && &actual[source_language.len()..] == COMPOSITION_SCHEMA_SUFFIX
        {
            Ok(())
        } else {
            Err(self.error(format!(
                "field {field:?} must match source language and checked composition schema, got {actual:?}"
            )))
        }
    }
